...
* 0 - Test
* 1 - Reference
* 2 - AECM 1500 per hectare
* 3 - AECM 2500 per hectare
* 4 - AECM 5000 per hectare
...
For the AECM: the subsidy cannot be put on the price of land because this would lead the Agriculture sector to try to use more land because it is cheaper
So the solution is to set the subsidy as a decrease on output price to make the value cheaper, but at the same time reducing the land availability with a percentage
defined outside the CGE model and based on the subsidy level. The subsidy should be endogenous and define a payment based on the payment per acre multiplied by the amount of acres that are reserved for ecoschemes

For EcoScheme that last year by year you can assume that farmers are more confident in giving land and give land to a level that the net gain is zero compared to reference for agriculture in terms of Value Added
So you can see what happens to the rest of the economy when the subsidy is covering the loss just enough rather than creating a margin

####################################### ToDo ##################################################################
Look at the "shocks to model sheet" in the Agricultural_land_belgium_Flanders_DataPattern.xlsx file.

FIRST OF ALL READJUST THE GDP GROWTH FOR FLANDERS

THEN
1- Define a payment f.ex 1445 € per acre
2- Define the payment depending on the land that the Farmers allocate (based on Allocated land percentage and total land, which depends on the real growth of land which now you have factored in
3- Define the endogenous subsidy and the constraint to ensure that the subsidy gives rise to a predefined payment based on the total allocated land and the defined subsidy per acre.
################################################################################################################
if growL = 1 then a multiplier of 3 is used on Eco to use in the computation of Lred to have more land allocated
for sustainable practices but with the same revenues as in the reference scenario for the agriculture
when the payment is set at 500 € per acre they basically get 500 per acre but give as much land as for 1500



Second point is the following: After a subsidy is decided do farmers adhere to the AECM immediately or does it take time?
If it happens immediately then I cannot reduce the land availability with a percentage over time but I need to do it immediately when the AECM is accepted.
--> Let us assume that the AECM is accepted in 2025 with a subsidy of : the value of land available
...
scalar Eco AECM for Farmers                                                                                                      /0/;
scalar growL  if set to 1 the measure is an EcoScheme (larger land reduction) set to 2 if doing bridging                         /0/;
scalar FP total factor productivity for the agricultural sector (bridging)                                                       /1/;
$endif
...
scalar GSus green subsidy                                                                                                        /0/;
scalar ExEff Extra energy efficiency                                                                                             /0/;
scalar Eco AECM for Farmers (payment per hectacre - Upper Bound 19704€ per hectacre)                                             /0/;
scalar growL  if set to 1 the measure is an EcoScheme (larger land reduction) set to 2 if doing bridging                         /0/;
scalar FP total factor productivity for the agricultural sector (bridging)                                                       /1/;
*scalar FP total factor productivity for the agricultural sector (bridging)    1500                                               /1.001232031/;
*scalar FP total factor productivity for the agricultural sector (bridging)   2500                                               /0.999272827/;
*scalar FP total factor productivity for the agricultural sector (bridging)   5000                                              /0.96191875/;
* For the Bridging one must change Eco, FP, e green
$endif
...
scalar GSus green subsidy                                                                                                        /0/;
scalar ExEff Extra energy efficiency                                                                                             /0/;
scalar Eco AECM for Farmers (payment per hectacre - Upper Bound 19704€ per hectacre)                                             /1500/;
scalar growL  if set to 1 the measure is an EcoScheme (larger land reduction) set to 2 if doing bridging                         /0/;
scalar FP total factor productivity for the agricultural sector (bridging)    1500                                               /1.001232031/;
* For the Bridging one must change Eco, FP, e green
$endif
...
scalar GSus green subsidy                                                                                                        /0/;
scalar ExEff Extra energy efficiency                                                                                             /0/;
scalar Eco AECM for Farmers (payment per hectacre - Upper Bound 19704€ per hectacre)                                             /2500/;
scalar growL  if set to 1 the measure is an EcoScheme (larger land reduction) set to 2 if doing bridging                         /0/;
scalar FP total factor productivity for the agricultural sector (bridging)   2500                                               /0.999272827/;
* For the Bridging one must change Eco, FP, e green
$endif
...
scalar GSus green subsidy                                                                                                        /0/;
scalar ExEff Extra energy efficiency                                                                                             /0/;
scalar Eco AECM for Farmers (payment per hectacre - Upper Bound 19704€ per hectacre)                                             /5000/;
scalar growL  if set to 1 the measure is an EcoScheme (larger land reduction) set to 2 if doing bridging                         /0/;
scalar FP total factor productivity for the agricultural sector (bridging)   5000                                              /0.96191875/;
* For the Bridging one must change Eco, FP, e green
$endif
...
TradeDatat(com0,'Fd','Fd')=0;
TradeDatat(com0,'RoBE','RoBE')=0;
...
* from Renting Price per ha.xlsx
...
ConsEneSHt(countries,source4,year)=
PrimaryEnergy('GENeSYS-MOD2.9','GradualDevelopment1.0',countries,source4,'EJ/yr',year,'value');
*AbsolutelyErase!!!  This is to test if the difference in the first 4 periods come because of the external database
ConsEneSHt('Switzerland','FinalEnergy|ResidentialandCommercial|Electricity','2020')=0.02;
$endif
...
SelEnet(countries,source1,year)=
PrimaryEnergy('GENeSYS-MOD2.9','Techno-Friendly1.0',countries,source1,'EJ/yr',year,'value');
ConsEneIndt(countries,source2,year)=
PrimaryEnergy('GENeSYS-MOD2.9','Techno-Friendly1.0',countries,source2,'EJ/yr',year,'value');
ConsEneTrat(countries,source3,year)=
PrimaryEnergy('GENeSYS-MOD2.9','Techno-Friendly1.0',countries,source3,'EJ/yr',year,'value');
ConsEneSHt(countries,source4,year)=
PrimaryEnergy('GENeSYS-MOD2.9','Techno-Friendly1.0',countries,source4,'EJ/yr',year,'value');
$endif
...
SelEnet(countries,source1,year)=
PrimaryEnergy('GENeSYS-MOD2.9','SocietalCommitment1.0',countries,source1,'EJ/yr',year,'value');
ConsEneIndt(countries,source2,year)=
PrimaryEnergy('GENeSYS-MOD2.9','SocietalCommitment1.0',countries,source2,'EJ/yr',year,'value');
ConsEneTrat(countries,source3,year)=
PrimaryEnergy('GENeSYS-MOD2.9','SocietalCommitment1.0',countries,source3,'EJ/yr',year,'value');
ConsEneSHt(countries,source4,year)=
PrimaryEnergy('GENeSYS-MOD2.9','SocietalCommitment1.0',countries,source4,'EJ/yr',year,'value');
$endif
...
scalar Tlnd Total land available in base year 2007 (hcr) /602177/;
parameter Lred Land reduction;
parameter pay Payment to farmers;
parameter TlndF fix the amount of land that is used to compute the payment for AECM in the future at the moment of the inception of the subsidy;
...
IOanalysis('input',cnt,sec,"Capital","1")=Pathway_adj_cap(cnt, sec)+AOGR(cnt)$(sameas(sec,"iCOI"))+ACR(cnt)$(sameas(sec,"iCOA"))+ANGR(cnt)$(sameas(sec,"iNG"))+ALR(cnt)$(sameas(sec,"iAGR"));
...
SAMout(cnt,"Capital",sec,"1")=Pathway_adj_cap(cnt, sec)+AOGR(cnt)$(sameas(sec,"iCOI"))+ACR(cnt)$(sameas(sec,"iCOA"))+ANGR(cnt)$(sameas(sec,"iNG"))+ALR(cnt)$(sameas(sec,"iAGR"));
...
SAMout(cnt,"HOUS","Capital","1")=KSZ(cnt)+AOGR(cnt)+ACR(cnt)+ANGR(cnt)+ALR(cnt);
...
Quantity(cnt,"Capital",sec,"1")=Pathway_adj_cap(cnt, sec)+AOGR(cnt)$(sameas(sec,"iCOI"))+ACR(cnt)$(sameas(sec,"iCOA"))+ANGR(cnt)$(sameas(sec,"iNG"))+ALR(cnt)$(sameas(sec,"iAGR"));
...
Quantity(cnt,"HOUS","Capital","1")=KSZ(cnt)+AOGR(cnt)+ACR(cnt)+ANGR(cnt)+ALR(cnt);
...
*##################### AECM Calculations #################################
*$ontext
parameter green(step) greening land allocation taken from the AMT-CGE excel file under total greening 1500 /
1        0
2        0
3        0
4        0
...
*$offtext
$ontext
parameter green(step) greening land allocation taken from the AMT-CGE excel file under total greening 2500 /
1        0
2        0
3        0
4        0
...
$offtext
$ontext
parameter green(step) greening land allocation taken from the AMT-CGE excel file under total greening 5000 /
1        0
2        0
3        0
4        0
...
* Formula based on data in Agricultural_land_Belgium_Flanders_DataPattern.xlsx
* Set to value only if formula larger than zero.
Lred$(growL eq 0 and Eco*1/1000*0.0545-0.0739 gt 0) = Eco*1/1000*0.0545-0.0739;
Lred$(growL eq 1 and Eco*3/1000*0.0545-0.0739 gt 0) = Eco*3/1000*0.0545-0.0739;
* *bridging* with bridging we set the growL flag at 2, this will make the value of the allocated land given by the external data taken by UCM
Lred$(growL eq 2)=0;
*Lred is the percentage decrease in land
*Tlnd is the total amount of land
*(THIS NEEDS TO UPDATE DEPENDING ON WHEN THE SCHEME STARTS BECAUSE THE AMOUNT
*OF LAND WILL CHANGE)
*Eco is the payment
...
execute_unload "shocks.gdx" Pathway_adj, Pathway_adj_cap, KZ, Pathway_total, gdp_p,growth_p,T_Adj,newshare,ty,PRC, Rp, OGR,AOGR,CR,ACR,NGR,ANGR,LR,ALR,gdp_p,eint,cint,ROWd;
...
SAMout(cnt,"Capital",sec,step)=REP_Capital.L(cnt, sec)*RKC.L(cnt)/PU.l(cnt)+POG.L(cnt)*AOGR(cnt)$(sameas(sec,"iCOI"))/PU.L(cnt)+PCL.L(cnt)*ACR(cnt)$(sameas(sec,"iCOA"))/PU.L(cnt)+PNG.L(cnt)*ANGR(cnt)$(sameas(sec,"iNG"))/PU.L(cnt)+PLR.L(cnt)*ALR(cnt)$(sameas(sec,"iAGR"))/PU.L(cnt);
...
SAMout(cnt,"HOUS","Capital",step)=RKC.L(cnt)/PU.l(cnt)*KSZ0(cnt)*(1-ty(cnt))*gdp_p(cnt)+POG.L(cnt)*AOGR(cnt)/PU.L(cnt)+PCL.L(cnt)*ACR(cnt)/PU.L(cnt)+PNG.L(cnt)*ANGR(cnt)/PU.L(cnt)+PLR.L(cnt)*ALR(cnt)/PU.L(cnt);
...
Quantity(cnt,"Capital",sec,step)=REP_Capital.L(cnt, sec)+AOGR(cnt)$(sameas(sec,"iCOI"))+ACR(cnt)$(sameas(sec,"iCOA"))+ANGR(cnt)$(sameas(sec,"iNG"))+ALR(cnt)$(sameas(sec,"iAGR"));
...
Quantity(cnt,"HOUS","Capital",step)=KSZ0(cnt)*(1-ty(cnt))*gdp_p(cnt)+AOGR(cnt)+ACR(cnt)+ANGR(cnt)+ALR(cnt);
...
QQQ(cnt,"Capital",sec,step)$SAM(cnt,"Capital",sec)=(REP_Capital.L(cnt, sec)+AOGR(cnt)$(sameas(sec,"iCOI"))+ACR(cnt)$(sameas(sec,"iCOA"))+ANGR(cnt)$(sameas(sec,"iNG"))+ALR(cnt)$(sameas(sec,"iAGR")))/SAM(cnt,"Capital",sec)-1;
...
*                               (TotalValue/Q)/CPI)
PPP(cnt,"Capital",sec,step)$SAM(cnt,"Capital",sec)=SAMout(cnt,"Capital",sec,step)/(REP_Capital.L(cnt, sec)+AOGR(cnt)$(sameas(sec,"iCOI"))+ACR(cnt)$(sameas(sec,"iCOA"))+ANGR(cnt)$(sameas(sec,"iNG"))+ALR(cnt)$(sameas(sec,"iAGR")));
...
ALR(cnt)$(not sameas(cnt,'Fd'))=ALR(cnt)*(1+grow(cnt));

* For Flanders we need to have this growth based on real growth of land which is 0.0056 every five years.
ALR(cnt)$(sameas(cnt,'Fd'))=ALR(cnt)*(1+0.0056);
* This is land availability in hectacres.
Tlnd=Tlnd*(1+0.0056)
...
* the land grows by a general growth (this is not the growth of land but it can be part of its growth in productivity because in normal growth we make it grow like the economy)
ALR(cnt)$(not sameas(cnt,'Fd'))=ALR(cnt)*(1+grow(cnt));

* For Flanders we need to have this growth based on real growth of land which is 0.0056 every five years.
* This is land availability computed as monetary value
ALR(cnt)$(sameas(cnt,'Fd'))=ALR(cnt)*(1+0.0056);

* This is land availability in hectacres.
Tlnd=Tlnd*(1+0.0056);
* a Percentage of this land is allocated to AECM Just once. This means that if AECM are used in 2025 the farmers will reduce their land by a given percentage
* the rest will go back growing but that percentage is removed once

* ############ *BRIDGING* BASED ON DATA PROVIDED BY UCM ALR WILL KEEP CHANGING EVERY 5 YEARS (IT CAN EVEN RE-INCREASE)
* in case of bridging...
Lred$(growL eq 2)=green(step);
* without bridging
ALR(cnt)$(Eco gt 0 and sameas(cnt,'Fd') and ord(step) eq 5)=ALR(cnt)*(1-Lred);

*Save the value of land in the moment that the payment is done. The payment is done 'forever' on that amount. It will not grow in time.
TlndF$(ord(step) eq 5)=Tlnd;
pay$(Lred gt 0 and ord(step) ge 5) = Lred*TlndF*Eco/1000000;
display pay, Tlnd, Lred;
* if The AECM only lasts five years we can use the following back adjustment
*ALR(cnt)$(Eco gt 0 and sameas(cnt,'Fd') and ord(step) eq 6)=ALR(cnt)/(1-Lred);
...
$offtext
* Final output
execute_unload "outputREMES.gdx" outAct,GDPout,VA,UNEMPLOYMENT,IOanalysis,SAM,SAMout,PPP,Quantity,PriceCO2,gdptest,gdp,EEdem,qKLEM,pKLEM,fKLEM,EUIMP,EUEXP;
...
KPI_Agr Sales of agriculture in 2050 relative to 2025;
...
KPI_Agr=Quantity("Fd",'iAGR',"pAGR",'10')/Quantity("Fd",'iAGR',"pAGR",'4');

execute_unload "compareREMESEXIOMOD.gdx" Gout,OIVo,OIVa,OEVo,OEVa,OEnVo,OEnVa,PriceCO2,Emis;

execute_unload "KPIs.gdx"  KPI_E,KPI_O,KPI_G,KPI_Coa,KPI_Bio,KPI_Ful,KPI_Dsl,KPI_Gsl,KPI_H2E,KPI_GDP,cint,KPI_Cons,KPI_Agr;
