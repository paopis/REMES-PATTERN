...
$AUXILIARY:
        ...
        SEco(cnt)                                        ! (Endogenous) Subsidy for EcoSchemes
...
*Domestic Production
$PROD:XD(cnt,sec)$(XDZ(cnt,sec) and P_S > 0 and not sameas(sec,"iH2S") and not sameas(sec,"iH2C") and not sameas(sec,"iH2E") and not sameas(sec,"iELC") and not ressecs(sec) and not sameas(sec,"iAGR"))
...
O:PD(cnt,sec,com)$(XDDZ(cnt,sec,com))       Q:((XDDZ(cnt,sec,com))*(1$(not sameas(cnt,'Fd'))+FPs$sameas(cnt,'Fd')))
+        P:((1-taxpz(cnt,sec))/(1$(not sameas(cnt,'Fd'))+FPs$sameas(cnt,'Fd')))
+        A:GOVTL(cnt)   T:taxp(cnt,sec)  A:GOVTL(cnt)  N:SEco(cnt)$sameas(cnt,'Fd')   M:(-1)$sameas(cnt,'Fd')
...
* this is not quite correct. Just to test;
* SEco is the percent subsidy
* PD is selected to be the output of the agriculture products (1 at the baseline)
* XD is the activity level (1 at the baseline)
* XDZ is the benchmark activity level for agriculture sector
$CONSTRAINT:SEco(cnt)$(not sameas(cnt,'Fd'))
SEco(cnt)=e=0;

$CONSTRAINT:SEco(cnt)$(sameas(cnt,'Fd'))
SEco(cnt)*sum((com,sec)$(sameas(com,'pAGR') and sameas(sec,'iAGR')),PD(cnt, sec, com)*XDDZ(cnt,sec,com)*XD(cnt,sec)) =e= pay;

*SEco(cnt)*sum((com,sec)$(sameas(com,'pAGR') and sameas(sec,'iAGR')),PD(cnt, sec, com)*XD(cnt,sec)*XDZ(cnt,sec))=e=pay;
