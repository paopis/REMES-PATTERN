...
*Domestic Production
$PROD:XD(cnt,sec)$(XDZ(cnt,sec) and P_S > 0 and not sameas(sec,"iH2S") and not sameas(sec,"iH2C") and not sameas(sec,"iH2E") and not sameas(sec,"iELC") and not ressecs(sec) and not sameas(sec,"iTRL"))
...
*Domestic Production
$PROD:XD(cnt,sec)$(XDZ(cnt,sec) and P_S > 0 and sameas(sec,"iTRL") and not sameas(cnt,"Pi"))
+ t:0 s:ELAS(sec, "KLEM") sM(s):0 skle(s):ELAS(sec, "KLE")  sE(sKLE):0.1$(ES0 eq 1)  sE(sKLE):5$(ES0 eq 0) skl(skle):ELAS(sec, "KL") sk(skl):10 com.tl(sE):0  sEl(sE):0
O:PD(cnt,sec,com)$(XDDZ(cnt,sec,com))       Q:(XDDZ(cnt,sec,com))
+        P:(1-taxpz(cnt,sec))
+        A:GOVTL(cnt)   T:taxp(cnt,sec)
...
*Domestic Production
$PROD:XD(cnt,sec)$(XDZ(cnt,sec) and P_S > 0 and sameas(sec,"iTRL") and sameas(cnt,"Pi"))
+ t:0 s:ELAS(sec, "KLEM") sM(s):0 skle(s):ELAS(sec, "KLE")  sE(sKLE):0.1$(EST eq 1)  sE(sKLE):5$(EST eq 0) skl(skle):ELAS(sec, "KL") sk(skl):10 com.tl(sE):0  sEl(sE):0
O:PD(cnt,sec,com)$(XDDZ(cnt,sec,com))       Q:(XDDZ(cnt,sec,com)*FPs)
+        P:((1-taxpz(cnt,sec))/FPs)
+        A:GOVTL(cnt)   T:taxp(cnt,sec)
...
* Labour, Capital & Savings
I:PL(cnt)               Q:(LZ(cnt,sec)/(PRC(cnt)*(1+CPD(cnt)$GWsec(sec)))) P:(PRC(cnt)*(1+CPD(cnt)$GWsec(sec)))     skl:
I:RKC(cnt)$(Pathway_adj_cap(cnt, sec))        Q:(Pathway_adj_cap(cnt, sec)*(CIRL(sec)$(ES ge 2)+1$(ES lt 2)))    sk:
I:PS(cnt)               Q:INVZ(cnt,sec)  sk:
...
+        P:((1+taxcz(cnt,com))/eintFC(cnt))       A:GOVTL(cnt)       T:taxc(cnt,com)
+ sEl:
I:P(cnt,com)$(P_S > 0 and FF(com)=1)   Q:((CZ(cnt,com) + Pathway_adj(cnt, "HOUS", com))*eintFC(cnt))
+        P:((1+taxcz(cnt,com))/eintFC(cnt))        A:GOVTL(cnt)       T:taxc(cnt,com)
+        com.tl:
...
* this was to fix the reference scenario of the Piedmont case because pTRD was skyrocketing in the reference scenario bringing the exports of everything down.
*Domestic/Regional/International production split
$PROD:EXPORT(cnt,com)$( sum(sec, XDDZ(cnt,sec,com) ) and not sameas(com,"pH2") and not (sameas(com,"pTRD") and sameas(cnt,"Pi"))) t:1.4$(worldcom(com)=0) t:5$(worldcom(com)=1) s:1.2
...
$PROD:EXPORT(cnt,com)$( sum(sec, XDDZ(cnt,sec,com) ) and not sameas(com,"pH2") and (sameas(com,"pTRD") and sameas(cnt,"Pi"))) t:0$(worldcom(com)=0) t:5$(worldcom(com)=1) s:1.2
O:ERint#(cntt)                                                           Q:(TRADEZ(com,cnt,cntt))
O:PDD(cnt,com)$(XXDZ(cnt,com)>1e-6)                                      Q:XXDZ(cnt,com)
O:ERext                                                                  Q:(EROWZ(cnt,com)*ROWgt*(1+ROWc$sameas(com,'pCOA')))   P:(1/(1+ROWc$sameas(com,'pCOA')))
I:PD(cnt,sec,com)$(XDDZ(cnt,sec,com))                                    Q:(XDDZ(cnt,sec,com))
